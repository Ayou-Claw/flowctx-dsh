// Vendored from lossless-claw (MIT). SQLite connection + busy-timeout stack;
// node:sqlite + node builtins only.
//
 ...
 *
 * Connections are tracked so tests can close them by path via closeDatabaseConnection().
 */
export function createDatabaseConnection(dbPath: string): DatabaseSync {
 ...
 */
export function closeDatabaseConnection(target?: string | DatabaseSync): void {
...
export const getDatabaseConnection = createDatabaseConnection;
