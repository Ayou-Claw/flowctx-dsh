// Vendored from lossless-claw (MIT). Per-database async transaction mutex.
//
/**
 * Per-database async transaction mutex.
 *
 * Hotfix for a nested-transaction concurrency bug (upstream issue #260).
 *
...
function nextSavepointName(): string {
  nextSavepointId += 1;
  return `txn_savepoint_${nextSavepointId}`;
}
